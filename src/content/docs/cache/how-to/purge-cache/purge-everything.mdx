...
Purge Everything deletes the resource, resulting in the `CF-Cache-Status` header indicating [`MISS`](/cache/concepts/cache-responses/#miss) for subsequent requests. To mark all cached content as stale instead, [invalidate](/cache/guides/invalidate-cache/) it.
